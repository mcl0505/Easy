...
object MoshiUtils {
    ...
    //高效序列化为map
    inline fun <reified K, reified V> mapFromJson(json: Any): MutableMap<K, V> = fromJson(
        json.toJsonString(),
...
//快捷序列化  json 字符串转为Map 集合
inline fun  <reified K, reified V> String.jsonToMap() = MoshiUtils.mapFromJson<K, V>(this)

//快捷序列化  json 字符串转为List 集合
inline fun  <reified T : Any> String.jsonToList() = MoshiUtils.listFromJson<T>(this)

//快捷序列化  json 字符串转为对象
inline fun <reified T : Any> String.jsonToBean() = MoshiUtils.fromJson<T>(this)

//快捷反序列化  将对象转为json 格式字符串
fun Any.toJsonString() = MoshiUtils.toJson(this)